...
const SOCKET_SERVER_URL = "https://sever-final-project.onrender.com/"; // Replace with your server URL
...
const NotificationItem = ({ notify, fetchNotifications }: { notify: any; fetchNotifications: () => void }) => {
    ...
    return (
        <div
            onClick={handleClick}
            className={`cursor-pointer border-b ${status ? "bg-slate-100" : ''}`}
        >
...
const NotificationList = () => {
    ...
    useEffect(() => {
        ...
        const socket = socketIOClient(SOCKET_SERVER_URL, {
            auth: {
                token: localStorage.getItem('accessToken'), // If you need to send auth token
            },
        });

        // Listen for 'newPlayerNotification' events
        socket.on("newPlayerNotification", () => {
    ...
    return (
        <div className="relative">
            <div
                ...
                className="cursor-pointer shadow-md w-[45px] border flex flex-col justify-center rounded-full h-[45px]"
            >
