...
export default function TopPlayer() {
    ...
    const getTop5Player = async () => {
        try {
            const res = await RentalRequestService.getTop5PlayersWithMostRentalRequests(selectedMonth, selectedYear)
    ...
    return (
        <div className="flex flex-col  py-6 justify-center items-center w-full bg-white shadow-lg rounded-lg px-4 border">
            <h1 className="font-semibold text-2xl tracking-wider uppercase text-center">Top 5 Most Hired Players</h1>
            <div className='py-2'>
                        ...
                        <TableBody>
                            {data.map((row, index) => (
                                ...
                                >
                                    ...
                                    <TableCell>{row.player.email}</TableCell>
                                    <TableCell>{row.rentalRequestsCount}</TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                    </Table>
